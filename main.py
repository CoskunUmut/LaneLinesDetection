...
while(cap.isOpened()):
    ...
    # # 1.1 Thresholding
    _, result = cv2.threshold(img1, 200, 255, cv2.THRESH_BINARY)

    # # # 2 Canny
    result = cv2.Canny(result, 50, 200)
    canny = result
    # RoI Left
    ...
    try:
        result_left = helpers.hough_lines(
            result_left, 1, np.pi/180, 15, 25, 75)  # hier noch interpolieren
        result_right = helpers.hough_lines(
            result_right, 1, np.pi/180, 15, 25, 75)  # hier noch interpolieren
        # # 4 GrayScale
        ...
        result = helpers.weighted_img(result_right, img1, 1, 1.0)
        result = helpers.weighted_img(result_left, result, 1, 1.0)
